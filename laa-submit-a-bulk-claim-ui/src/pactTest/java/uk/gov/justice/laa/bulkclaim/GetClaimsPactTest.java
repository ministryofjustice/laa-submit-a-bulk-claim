...
public final class GetClaimsPactTest extends AbstractPactTest {
  ...
  public RequestResponsePact getClaims200(PactDslWithProvider builder) {
    ...
    return builder
        .given("a claim exists for the search criteria")
        .uponReceiving("a search request for claims")
        .path("/api/v0/claims")
  ...
  public RequestResponsePact getClaimsEmpty200(PactDslWithProvider builder) {
    ...
    return builder
        .given("no claim exist for search criteria")
        .uponReceiving("a search request for claims that returns no results")
        .path("/api/v0/claims")
        .matchQuery("office_code", "([A-Z0-9]{6})")
        .matchQuery("submission_id", UUID_REGEX)
        .matchQuery("page", "([0-9]+)")
